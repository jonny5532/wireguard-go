...
func (net *Net) DialContextTCP(ctx context.Context, addr *net.TCPAddr) (*gonet.TCPConn, error) {
	...
	return net.DialContextTCPAddrPort(ctx, netip.AddrPortFrom(a, uint16(addr.Port)))
}
...
func (net *Net) DialTCP(addr *net.TCPAddr) (*gonet.TCPConn, error) {
	...
	a, _ := netip.AddrFromSlice(addr.IP)
	return net.DialTCPAddrPort(netip.AddrPortFrom(a, uint16(addr.Port)))
}
...
func (net *Net) ListenTCP(addr *net.TCPAddr) (*gonet.TCPListener, error) {
	...
	a, _ := netip.AddrFromSlice(addr.IP)
	return net.ListenTCPAddrPort(netip.AddrPortFrom(a, uint16(addr.Port)))
}
...
func (net *Net) DialUDP(laddr, raddr *net.UDPAddr) (*gonet.UDPConn, error) {
	...
	if laddr != nil {
		a, _ := netip.AddrFromSlice(laddr.IP)
		la = netip.AddrPortFrom(a, uint16(laddr.Port))
	}
	if raddr != nil {
		a, _ := netip.AddrFromSlice(raddr.IP)
		ra = netip.AddrPortFrom(a, uint16(raddr.Port))
	}
...
func (pc *PingConn) ReadFrom(p []byte) (n int, addr net.Addr, err error) {
	...
	a, _ := netip.AddrFromSlice([]byte(res.RemoteAddr.Addr))
	addr = &PingAddr{a}
